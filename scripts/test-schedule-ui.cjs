...
let revision=0,advance=30,retry=7,concurrency=3,failSave=false,saves=0;
const errors=[],checks=[];
const status=()=>({version:'0.3.4',mode:'force',valid_count:1,probe_enabled:true,continuous:true,retry_seconds:retry,refresh_before_seconds:advance*60,reasoning_effort:'medium',proxy_mode:'credential',schedule:{revision,advance_minutes:advance,retry_seconds:retry,default_advance_minutes:30,default_retry_seconds:7,proxy_concurrency:concurrency,default_proxy_concurrency:3,persistent:true},selection:{required:true,persistent:true,revision:0,models:['gpt-6-astra'],model_options:['gpt-6-astra'],accounts:[{account:'demo',email:'demo@example.test',selected:true}]},entries:[{account:'demo',model:'gpt-6-astra',selected:true,state_length:332,valid:true,expires_at:'2026-09-20T12:21:42Z',next_refresh_at:new Date(Date.parse('2026-09-20T12:21:42Z')-advance*60000).toISOString()}],history:[]});
 ...
 try{
  ...
  await page.route('**/v0/management/codex-turn-state-manager/**',async route=>{
   ...
   if(req.url().endsWith('/schedule')){
    ...
    advance=body.advance_minutes;retry=body.retry_seconds;concurrency=body.proxy_concurrency;revision++;saves++;
  ...
  assert.equal(await page.locator('#advanceMinutes').inputValue(),'30');assert.equal(await page.locator('#retrySeconds').inputValue(),'7');assert.match(await page.locator('#accounts').textContent(),/19:51:42/);assert.equal(await page.locator('#proxyConcurrency').inputValue(),'3');checks.push('defaults_and_expected_refresh');
  await page.locator('#advanceMinutes').fill('10');await page.locator('#retrySeconds').fill('11');await page.locator('#proxyConcurrency').fill('2');await page.getByRole('button',{name:'刷新',exact:true}).click();
  assert.equal(await page.locator('#advanceMinutes').inputValue(),'10');assert.equal(await page.locator('#retrySeconds').inputValue(),'11');assert.equal(await page.locator('#proxyConcurrency').inputValue(),'2');checks.push('poll_preserves_unsaved_inputs');
  ...
  await page.reload();await page.locator('#scheduleNote').filter({hasText:'到期前 10 分钟'}).waitFor();assert.equal(await page.locator('#retrySeconds').inputValue(),'11');assert.equal(concurrency,2);assert.equal(await page.locator('#proxyConcurrency').inputValue(),'2');checks.push('reload_restores_values');
  ...
  await page.locator('#resetSchedule').click();await page.locator('#scheduleNote').filter({hasText:'到期前 30 分钟'}).waitFor();assert.equal(advance,30);assert.equal(retry,7);assert.equal(await page.locator('#retrySeconds').inputValue(),'7');assert.equal(concurrency,3);assert.equal(await page.locator('#proxyConcurrency').inputValue(),'3');checks.push('restore_defaults_persists');
  await page.setViewportSize({width:390,height:844});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.locator('.schedule-panel').screenshot({path:path.join(root,'dist','schedule-ui-0.3.4.png')});assert.deepEqual(errors,[]);checks.push('mobile_layout_and_no_errors');
