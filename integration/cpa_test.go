...
func TestRealCPANativeHotUpgradeSelection(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(h.root, "plugins", "codex-turn-state-manager-v0.3.4.so"), data, 0600); err != nil {
		t.Fatal(err)
	}
	patch, _ := json.Marshal(map[string]any{"selection_required": true, "selection_file": filepath.Join(h.root, "selection.json")})
	...
	await(t, func() bool {
		...
		return code == 200 && json.Unmarshal(body, &state) == nil && state.Version == "0.3.4" && state.Selection.Required
	})
}
